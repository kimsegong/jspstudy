...
public class Download extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  // 다운로드용 응답을 만드려면 응답 헤더 Content-Disposition에 "attachment" 값을 저장해야 한다.
	  response.setHeader("Content-Disposition", "attachment"); // 다운로드할 때 파일 저장 대화상자가 나타난다.
	  response.setHeader("Contect-Disposition", "attachment; filename=" + path.substring(path.lastIndexOf("\\") + 1)); // 다운로드할 때 작성한 filename으로 곧바로 저장된다.
